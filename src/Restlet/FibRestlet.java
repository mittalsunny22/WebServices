...
public class FibRestlet {
	...
	private void publish_service() {
		try {
			...
			// Attach a handler to handle client requests. (Note the similarity of the
			// handle method to an HttpServlet method such as doGet or doPost.)
			Restlet handler = new Restlet(component.getContext()) {
				...
				public void handle(Request req, Response res) {
					...
					if (http_verb.equals(Method.GET)) {
						...
						Form form = req.getEntityAsForm(); // it means we need to add data in body as
						...
						String xml = xml_start + "Resource deleted" + xml_stop;
						res.setStatus(Status.SUCCESS_OK);
						res.setEntity(xml, MediaType.APPLICATION_XML);
